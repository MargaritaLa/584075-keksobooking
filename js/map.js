(function () {
  ...
  var objectPhotos = [];
  var cardObjectsArray = createObjectsArray(cardObjectsCount);
 ...
 /*  выводим все объекты перед блоком .map__filters-container */
 function renderObjects(blockMap, cardObjectsArray) {
  ...
  for (var i = 0; i <= cardObjectsArray.length - 1; i++) {
    blockMap.insertBefore(renderObject(mapPopupObjectTemplate, cardObjectsArray[i]), lookoutEl);
  }
}
...
function renderPins(container, objectsArray) {
  ...
  for (var i = 0; i <= objectsArray.length - 1; i++) {
    container.appendChild(renderPin(mapPinsObjectTemplate, objectsArray[i]));
  }
}
